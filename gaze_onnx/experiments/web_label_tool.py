...
def load_manifest(samples_dir: Path) -> List[Sample]:
    ...
    # stable ordering
    rows.sort(key=lambda row: (str(row.get("Domain", "")), int(float(row["FrameID"]))))
            ...
            try:
                ...
                if path.startswith("/img/"):
                    ...
                    if not img_path.exists() or img_path.suffix.lower() not in (".png", ".jpg", ".jpeg"):
                        ...
                        return
                    # ensure it stays within samples_dir
                ...
                if path.startswith("/item/"):
                    ...
                    body = "".join(
                        [
                            ...
                            f"<div><span class='k'>Pred:</span> {html.escape(s.pred_class)} &nbsp; <span class='k'>Label:</span> {html.escape(current_label or '(unlabeled)')}</div>"
