...
from autodri.gaze.gaze_state_cls import SCRFDDetector, face_area_ratio
...
def main() -> None:
    ...
    if args.gaze_cls_model == "models/gaze_cls_yolov8n.onnx":
        args.gaze_cls_model = str(
            resolve_existing_path(
                "",
                workspace_rel="models/gaze_cls_yolov8n.onnx",
                legacy_rels=("models/gaze_cls_yolov8n.onnx",),
                description="gaze classifier model",
            )
        )
    if args.assignment_csv == "gaze_onnx/experiments/output/dual_roi_assignment.csv":
        args.assignment_csv = str(workspace_root(create=True) / "artifacts" / "assign_dual_roi" / "dual_roi_assignment.csv")
    if args.run_out_dir == "gaze_onnx/experiments/output/dual_roi_runs":
        args.run_out_dir = str(workspace_root(create=True) / "artifacts" / "assign_dual_roi" / "runs")
