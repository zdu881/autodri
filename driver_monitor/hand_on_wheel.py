...
def select_roi_interactive(cap: cv2.VideoCapture) -> tuple:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="logs/hand_on_wheel.mp4",
    ...
    if args.roi:
        ...
        roi = select_roi_interactive(cap)
        if roi:
    ...
    while True:
        ...
        if frame_idx % stride == 0:
            ...
            decision = iou_max > args.iou_threshold
        ...
        status_text = "HAND ON WHEEL" if last_decision else "HAND OFF WHEEL"
        status_color = (0, 255, 0) if last_decision else (0, 0, 255)
